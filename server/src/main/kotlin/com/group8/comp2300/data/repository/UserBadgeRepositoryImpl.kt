...
class UserBadgeRepositoryImpl(private val database: ServerDatabase) : UserBadgeRepository {
    ...
    override fun hasUserEarnedBadge(userId: String, badgeId: String): Boolean =
        userBadgeQueries.hasUserEarnedBadge(userId, badgeId).executeAsOne()

    override fun getEarnedBadges(userId: String): List<EarnedBadge> =
        badgeQueries.getEarnedBadgesByUserId(userId).executeAsList().map { row ->
            EarnedBadge(
    ...
    override fun getLockedBadges(userId: String): List<Badge> =
        badgeQueries.getLockedBadgesByUserId(userId).executeAsList().map { row ->
            Badge(row.id, row.badge_name, row.badge_path)
        }

    override fun getEarnedBadgeCount(userId: String): Long =
        userBadgeQueries.getBadgeUserBadgeCount(userId).executeAsOne()
