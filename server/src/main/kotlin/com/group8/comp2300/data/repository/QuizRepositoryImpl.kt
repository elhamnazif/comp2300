...
class QuizRepositoryImpl(private val database: ServerDatabase) : QuizRepository {

    private val quizQueries = database.quizQueries

    override fun getQuizById(id: String): Quiz? = quizQueries.getQuizById(id).executeAsOneOrNull()?.let {
        mapToDomain(it)
    }

    override fun getQuizByArticleId(articleId: String): Quiz? =
        quizQueries.getQuizByArticleId(articleId).executeAsOneOrNull()?.let {
            mapToDomain(it)
        }

    override fun getAllQuizzes(): List<Quiz> = quizQueries.getAllQuizzes().executeAsList().map { mapToDomain(it) }
