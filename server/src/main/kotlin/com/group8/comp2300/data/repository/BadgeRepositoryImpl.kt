...
class BadgeRepositoryImpl(private val database: ServerDatabase) : BadgeRepository {
    ...
    override fun getAllBadges(): List<Badge> = badgeQueries.getAllBadges().executeAsList().map { row ->
        Badge(row.id, row.badge_name, row.badge_path)
    }

    override fun getBadgeById(badgeId: String): Badge? =
        badgeQueries.getBadgeById(badgeId).executeAsOneOrNull()?.let { row ->
            Badge(row.id, row.badge_name, row.badge_path)
        }

    override fun getBadgeByName(name: String): Badge? =
        badgeQueries.getBadgeByName(name).executeAsOneOrNull()?.let { row ->
            Badge(row.id, row.badge_name, row.badge_path)
        }
}
