...
class ArticleRepositoryImpl(
    ...
    override fun getArticleById(id: String): Article? {
        return articleQueries.getArticleById(id).executeAsOneOrNull()?.let { mapToDomain(it) }
    }

    override fun getAllArticles(): List<Article> {
        return articleQueries.getAllArticles().executeAsList().map { mapToDomain(it) }
    }
    ...
    override fun searchArticles(query: String): List<Article> {
        return articleQueries.searchArticles(query).executeAsList().map { mapToDomain(it) }
    }

    override fun getArticlesByCategory(categoryId: String): List<Article> {
        return articleCategoryQueries.getArticlesByCategory(categoryId).executeAsList().map { mapToDomain(it) }
    }
