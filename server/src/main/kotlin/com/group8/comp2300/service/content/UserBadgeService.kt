...
class UserBadgeService(
    ...
    fun checkForNewBadges(userId: String): List<String> {
        ...
        val rules = listOf(
            ...
            BadgeRule("Speed_Demon", condition = { avgSeconds > 0 && avgSeconds < 30.0 && perfectCount >= 1 })
        )
    ...
    fun getFullAchievementProfile(userId: String): List<EarnedBadge> {
        return userBadgeRepo.getEarnedBadges(userId)
    }
    fun getLockedAchievements(userId: String): List<Badge> {
        return userBadgeRepo.getLockedBadges(userId)
    }
