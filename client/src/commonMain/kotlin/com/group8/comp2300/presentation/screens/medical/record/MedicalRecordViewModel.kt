...
class MedicalRecordViewModel (
    private val repository: MedicalRecordRepository ) : ViewModel() {
    ...
    fun loadRecords(sort: RecordSortOrder = uiState.selectedSort) {
        viewModelScope.launch {
            ...
            try {
                val records = repository.getRecords(sort.apiValue)
    ...
    fun uploadFile(fileBytes: ByteArray, fileName: String) {
        ...
        viewModelScope.launch {
            uiState = uiState.copy(isLoading = true)
            val success = repository.upload(fileBytes, fileName)
            if (success) loadRecords()
            else uiState = uiState.copy(errorMessage = "Failed to upload records", isLoading = false)
        }
    }
