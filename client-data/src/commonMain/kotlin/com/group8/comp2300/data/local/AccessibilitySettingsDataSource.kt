...
data class AccessibilitySettings(val grayscaleEnabled: Boolean = false)

class AccessibilitySettingsDataSource(private val settings: Settings) {
    ...
    fun setGrayscaleEnabled(enabled: Boolean) {
        settings.putBoolean(KeyGrayscaleEnabled, enabled)
    ...
    private fun loadSettings(): AccessibilitySettings = AccessibilitySettings(
        grayscaleEnabled = settings.getBoolean(KeyGrayscaleEnabled, false),
    )

    private companion object {
        const val KeyGrayscaleEnabled = "accessibility.grayscale_enabled"
    }
}
