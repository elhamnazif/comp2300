...
fun UserDto.toDomain(): User {
    return User(
        ...
        gender = gender?.let { 
            try { Gender.valueOf(it.uppercase()) } catch (e: Exception) { Gender.PREFER_NOT_TO_SAY } 
        },
        sexualOrientation = sexualOrientation?.let { 
            try { SexualOrientation.valueOf(it.uppercase()) } catch (e: Exception) { SexualOrientation.PREFER_NOT_TO_SAY } 
        },
