...
To provide a single source of truth for the domain layer and any reusable logic that should be shared between the client (`client`) and the `server`.
...
This module is a dependency for `client`, `client-data`, and `server`. It ensures that both client and backend share the same core domain structures.
